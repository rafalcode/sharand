...
void hx(cairo_surface_t *surface, cairo_t *cr, float segl, float x0, float y0, int nsides)
{
    ...
    cairo_rel_move_to(cr, -segl*sin(pid2), segl*cos(pid2));

    // now we follow intuition, drawing straight line first, for Cairo we must negate the y.
    for(int i=0;i<nsides-1;++i)
        cairo_rel_line_to(cr, segl*cos(i*pid1), -segl*sin(i*pid1));
    cairo_close_path(cr);
    cairo_stroke(cr);
}
...
int main (int argc, char *argv[])
{
    ...
    float segl=100.;
    hx(surface, cr, segl, x0, y0, nsides);
